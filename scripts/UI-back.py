     ...
     def run(self):
         ...
         try:
           ...
           rewriteReport(diff_html_file)
            ...
            print('重写报告异常，但不影响报告的正确性，异常部分如下：')
            print(e)
...
class UI:
  ...
  def initWindow (window,scrText):
    ...
    #文件选择按钮
    tk.Button(window,text='浏览...',command=selectDirectory1).place(x=525,y=110);
    tk.Button(window,text='浏览...',command=selectDirectory2).place(x=525,y=150);
                ...
                if not newDir.endswith('report.html'):
                   ...
                   try:
                     ...
                     os.remove(newDir)
                      ...
                      ui.writeLog('warning:删除 :{}文件出错，请确保文件没有占用'.format(newDir))
...
if __name__ == "__main__":
    ...
    win = UI(window,scrText)
    win.initWindow(window,scrText)
